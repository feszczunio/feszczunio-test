...
<h1 align="center">
  Statik GraphQL Plugin
</h1>

This plugin was prepared to access manage the GraphQL interface. Based on the WP GraphQL package this plugin extends
those functionalities in the way, that allows replacing Gatsby's internal GraphQL with this one.
...
* provide GraphQL interface directly in the WP,
* present data from all Statik plugins.

## 🧐 How does the plugin work?

The plugin is response for save all Gutenberg data into the separate meta in the database and provide them to use in the
Gatsby application.
...
The development mode in the plugin can be activated by the constant `STATIK_GRAPHQL_DEVELOPMENT`. When the value of
the constant is set to true, it is loaded on the dashboard and not the minified asset version.
...
The Statik GraphQL Plugin is licensed under the GPL v3 or later.
